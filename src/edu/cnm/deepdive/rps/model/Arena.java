...
public class Arena {
  ...
  public void advance(int numIterations){
    for (int i = 0; i < numIterations ; i++) {
      synchronized (lock) {
        ...
        byte challenger = terrain[challengerRow][challengerRow];
  ...
  private int wrap(int value) {
    value %= arenaSize;  //value = value % arenaSize
    return (value >= 0) ? value : value + arenaSize;
  }

  private int compare(byte cell1, byte cell2) {  // attacker and defender
    int comparison;
    if (cell1 == cell2){
      comparison = 0;
    }else{
  ...
  private enum Direction{
    ...
    Direction(int rowOffset, int columnOffset) {    //default accessibility = private
